...
$EMAIL=~s/\*\*\*/\@/g;
if (!$EMAIL){$EMAIL="cedric.notredame\@europe.com";}
...
sub seq2tm_template
  ...
  foreach $seq (keys(%s))
	...
	{
	  system "t_coffee -other_pg fasta_seq2hmmtop_fasta.pl -in=seqfile -out=$lib_name -arch=$arch -psv=$psv";
	}
      elsif ( $mode eq "psitm_template")
	{
  ...
  foreach $seq (keys(%s))
    {
      ...
      $lib_name=&clean_file_name ($lib_name);
      system "t_coffee -other_pg RNAplfold2tclib.pl -in=seqfile -out=$lib_name";
...
sub seq2msa
  {
    ...
    `t_coffee -other_pg seq_reformat -in seq.pep -output fasta_seq > x`;
    `mv x seq.pep`;
...
sub seq2thread_pair
  {
	...
	`t_coffee -other_pg seq_reformat -in tmp_result.aln -output fasta_aln >result.aln`;
      }
    elsif ( $method eq "t_coffee")
      {
	`t_coffee -in Pstruc.pdb Sseq.pep Mslow_pair -outfile result.aln -quiet`;
      }
    else
      {
	`$method -infile=seq.pep -pdbfile1=struc.pdb -outfile=result.aln $param>x 2>x`;
      }
    &set_temporary_dir ("unset",$mode,$method,"result.aln",$outfile);
    myexit (EXIT_SUCCESS);
  }
	...
	if ($method eq "dalilite")
	  {
		...
		$command="wget 'http://www.ebi.ac.uk/Tools/es/cgi-bin/jobresults.cgi/dalilite/dalilite-$jobid/aln.html' -O output2 -o/dev/null";
		`$command`;
		if ( -e "output2")
		  {
		    my ($seq1, $seq2);
...
sub seq2pdb_pair
  {
	...
	`t_coffee -in Ppdb1.pdb Ppdb2.pdb -quiet -outfile=result.aln`;
      ...
      {
	if ( system ("TMalign pdb1.pdb pdb2.pdb >tmp1")==EXIT_SUCCESS)
	  {
	    `tail -4 tmp1 > tmp2`;
      ...
      {
	if ( system ("mustang -i pdb1.pdb pdb2.pdb -F fasta >/dev/null 2>/dev/null")==EXIT_SUCCESS)
	  {
      ...
      {
	if ( system ("$method -pdbfile1=pdb1.pep -pdbfile2=pdb2.pdb -outfile=result.aln $param>x 2>x")==EXIT_SUCCESS)
	  {
	    `mv results.afasta result.aln`;
	  }
	else
...
sub seq2msa_tm_prediction 
  {
    ...
    system "t_coffee -other_pg fasta_seq2hmmtop_fasta.pl -in=tm_input -out=tm_output -arch=$arch -psv=$psv";
    unlink ("tm_input");
...
sub run_blast
  {
    my ($name, $method, $db,$infile, $outfile)=(@_);
    
  
    
    if (&cache_file("GET",$name,$method,$db,$outfile,$SERVER)){return $outfile;}
    else
      {
	if ( $SERVER eq "EBI")
	  {
	    if ( $method =~/wu/)
	      {
		$method=~s/wu//;
		$command="t_coffee -other_pg wublast.pl --email $EMAIL $infile -D $db -p $method --outfile $outfile -o xml>/dev/null 2>/dev/null";
		system ( $command);`cp $outfile.xml $outfile`;
	      }
	    else
	      {
		if ($method eq "psiblast"){$cl_method ="PSI-Blast -j5";}
		else {$cl_method = $method;}
	...
	&cache_file("SET",$name,$method,$db,$outfile,$SERVER);
	return $outfile;
      }
  }
...
sub cache_file
  {
    my ($cache_mode,$name,$method,$db, $outfile,$server)=(@_);
    my $cache_file;
