...
def node2leaf (N,nodes, nlist):
    #missing This function should call itself in a recursive way
    #the idea is to start exploring left then rigth - or the other way round
    
    if nodes[N].name!="":
        # missing 1 - what to do when reaching an extremity
        return nlist
    else:
        if nodes[N].left:
            nlist=node2leaf(nodes[N].left, nodes, nlist)
        #missing 2 - two more lines
...
#print the list of all the leafs associated with a node
#use a recursive formulation: i.e. a function calling itself
for n in nodes:
    if n>0:
        nlist=[]
        nlist=node2leaf (n,nodes,nlist)
        print ("Node ", n, nlist)
