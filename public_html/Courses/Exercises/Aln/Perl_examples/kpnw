...
#   Evaluation of each position in linear time.
#   each sequence comes in a differrent file


#Parameters: aln1 aln2
...
# Read  sequences from two fasta format file:
...
for ($i=1; $i<=$len0; $i++)
	...
	for ($j=1; $j<=$len1; $j++)
		...
		#score calculation in linear time
		for ($n=0; $n<$n0; $n++){$col0{$res0[$n][$i-1]}++;}
		for ($m=0; $m<$n1; $m++){$col1{$res1[$m][$j-1]}++;}
		$nm=0;
		foreach $a (@symbol_list)
		  {
		    foreach $b (@symbol_list)
		      {
			
			if ( $a eq "-" || $b eq "-"){;}
			elsif ( $a eq $b)
			  {
			    $s+=$col0{$a}*$col1{$b}*$match;
			    $nm+=$col0{$a}*$col1{$b};
			  }
			else 
			  {
			    $s+=$col0{$a}*$col1{$b}*$mismatch;
			    $nm+=$col0{$a}*$col1{$b};
			  }
		      }
		  }
